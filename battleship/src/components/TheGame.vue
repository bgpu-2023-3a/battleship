  ...
  methods: {
    ...
    updateTheBoardsInfo() {
      const updateInfoFor = (player, boardInfoElement) => {
        ...
        if (`Поле ${nameEl.textContent}` !== name) {
          nameEl.textContent = `Поле ${name}`;
        }

        if (`Целые корабли: ${aliveShipsEl.textContent}` !== aliveShips) {
          aliveShipsEl.textContent = `Целые корабли: ${aliveShips}`;
        }
      };
